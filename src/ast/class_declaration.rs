...
pub fn operator_method_id(class_name: String, operator: Token) -> String {
    format!(
        "{}._$_op_{}",
        class_name,
...
impl AstNode for ClassDeclarationNode {
    fn setup(&mut self, ctx: MutRc<dyn Context>) -> Result<(), Error> {
        ...
        self.generics_ctx = Some(Scope::new_local(ctx.clone()));

        for field in &self.fields {
            field
                .type_
                .borrow_mut()
                .setup(self.generics_ctx.clone().unwrap())?;
        }
    ...
    fn type_check(&self, mut ctx: MutRc<dyn Context>) -> Result<TypeCheckRes, Error> {
        ...
        if ctx.borrow().is_frozen() {
            ...
            unsafe {
                this_type =
                    (&*(&this_type_any as *const dyn Any as *const MutRc<ClassType>)).clone();
            }
        } else {
        ...
        for field in self.fields.iter() {
            let type_ = field
                .type_
                .borrow()
                .type_check(self.generics_ctx.clone().unwrap())?;
                ...
                .contains_key(&field.identifier.clone())
            {
                ...
                stack_offset = this_type.borrow().field_offset(field.identifier.clone());
            }
        ...
        for method in self.methods.iter() {
            ...
            method.class_name = Some(this_type.borrow().name.clone());
            let has_self_arg = method.params.first().is_some()
                && method.params.first().unwrap().identifier == "self";
            if has_self_arg {
                let first_param = method.params.first().unwrap().clone();
