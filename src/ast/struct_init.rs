...
use crate::util::{intersection, keys_match, MutRc};
...
impl StructInitNode {
    fn field_types_hashmap(&self, ctx: MutRc<Context>) -> Result<HashMap<String, Rc<dyn Type>>, Error> {
...
impl Node for StructInitNode {
    ...
    fn type_check(&mut self, ctx: MutRc<Context>) -> Result<TypeCheckRes, Error> {
        let struct_type = ctx.borrow_mut().get_dec_from_id(&self.identifier)?.type_.clone();
        ...
        let (extra, fields, missing) = intersection(
            &instance_fields_hashmap, &type_fields_hashmap);

        if extra.len() > 0 {
            return Err(type_error(format!(
                "Unknown fields in struct initialization: {}",
                extra.iter().map(|s| s.clone()).collect::<Vec<String>>().join(", ")
            )));
        }
        ...
        if missing.len() > 0 {
            return Err(type_error(format!(
                ...
                missing.iter().map(|s| s.clone()).collect::<Vec<String>>().join(", ")
            )));
        }

        for field in fields {
            if !type_fields_hashmap.get(&field).unwrap()
